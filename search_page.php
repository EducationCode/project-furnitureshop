...
<div class="content">
  ...
  <div class="top">
    ...
    <ul>
        <div class="name_bar">
            <ul>
                <li class="furniture"><a href=""><h6><?php echo "Admin ". $row['firstname'] . ' ' ?></h6></a>
                    <ul>
    ...
    <ul>
      ...
      <?php
        ...
        if(empty($_GET["search"])){
            setcookie("search",1,time()+1000); 
          }else if(empty($_COOKIE[$_GET["search"]])){
              setcookie($_GET["search"], 1, time()+3600*24);
          }
  
        if(isset($_COOKIE[$_GET["search"]])){
            $count = $_COOKIE[$_GET["search"]] + 1;
            setcookie($_GET["search"], $count, time()+3600*24);
        }
      ...
      <div class="print">
        <?php 
        $stmt = $pdo->prepare("SELECT * FROM product where CONCAT(Product_type, Product_style, Product_info, Product_name) LIKE '%$keyword%' limit $offset,$record_show " );
        $stmt->execute();
        ?>
          <?php while($row = $stmt->fetch()){
            ...
            <div class="show">
              <div class="grop">
                ...
                <hr>
                <div class="cart-shop">
                <h5 style="color:grey;"><?=$row["Product_price"]?></h5>
...
<nav aria-label="Page navigation example">
    <ul class="pagination justify-content-center">
        ...
        <a class="page-link" href="?search=<?=$keyword?>&page=<?=$page-1?>" aria-label="Previous">
        ...
        <a class="page-link" href="?search=<?=$keyword?>&page=<?=$page+1?>" aria-label="Next">
