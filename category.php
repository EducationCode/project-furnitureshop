...
<?php
    ...
    if(empty($_COOKIE[$_GET["keyword"]])){
        setcookie($_GET["keyword"], 1, time()+3600*24);
    }

    if(isset($_COOKIE[$_GET["keyword"]])){
        $count = $_COOKIE[$_GET["keyword"]] + 1;
        setcookie($_GET["keyword"], $count, time()+3600*24);
    }
  $stmt = $pdo->prepare("SELECT * from product where product_type like '%$keyword%'"); 
  $stmt->execute();
  $num_produce=0;
...
<div class="print">
  <?php 
    $stmt = $pdo->prepare("SELECT * from product where product_type like '%$keyword%' limit $offset,$record_show" ); 
    $stmt->execute();
    while($row = $stmt->fetch()){
  ?>
  <div class="show">
